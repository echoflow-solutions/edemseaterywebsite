...
/** A menu row exactly as it is written out below. */
type MenuItemInput = {
  name: string;
  price: string;
  description: string;
...
export type MenuItem = MenuItemInput & {
  /** Stable, section-scoped identifier. The cart stores these, never prices. */
  id: string;
  sectionId: string;
  /**
   * Price in cents, or null when the item is priced by add-ons. Null-priced
   * items cannot be added to the cart until modifiers are built.
   */
...
/** "$33.60" -> 3360. Anything that is not a plain dollar amount returns null. */
const toCents = (price: string): number | null => {
...
const buildSection = (sectionId: string, items: MenuItemInput[]): MenuItem[] =>
  items.map((item) => ({
    ...item,
    sectionId,
    id: `${sectionId}-${slugify(item.name)}`,
...
export const menuData: Record<string, MenuItem[]> = {
  ...
  proteins: buildSection('proteins', [
  ...
  kidsMenu: buildSection('kidsMenu', [
  ...
  smallTrays: buildSection('smallTrays', [
...
/**
 * Resolve a menu item by id. The cart and, later, the checkout route use this
 * so prices are always read from here rather than trusted from the browser.
 */
